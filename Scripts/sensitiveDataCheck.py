# -*- coding: utf-8 -*-
"""
Created on Tue Oct 10 09:42:03 2023
...
# Defined list of sensitive data patterns

sensitive_patterns = [
    r'\b\d{3}-\d{2}-\d{4}\b',  # Social Security Number (SSN) pattern
    r'\b\d{3}\d{2}\d{4}\b',  # Additional Social Security Number (SSN) pattern
    r'\b(?:\d{4}-?){3}\d{4}\b',  # Credit card number pattern
    r'\b(?:\d{4}-?){2}\d{4}-?\d{4}\b',  # Additional credit card number pattern
    r'\b(?:\d{4}\s?){4}\b',  # Additional credit card number pattern
...
if file_type.lower() == 'text':
    ...
    file_path = input(
        "Enter the image file path (type 'no' if not available): ")
